...
public class MapRYarnDeployer extends YarnDeployer implements Deployer {
	...
	public String[] getRelativePaths(String versionNumber) {
		...
		for (String jar : ADDITIONAL_HADOOP_JARS) {
			...
			} else if (jar.contains("hadoop") && jar.contains("hadoop-mapreduce-client-core-*.jar")) {
				strBuilder = new StringBuilder().append(File.separator)
						.append("share").append(File.separator)
						.append("hadoop").append(File.separator)
		...
		strBuilder = new StringBuilder().append(File.separator).append("share").append(File.separator).append("hadoop").append(File.separator).append("hdfs").append(File.separator).append("lib").append(File.separator).append("htrace-core*.jar");
		relativeJarPaths[i] = strBuilder.toString();
