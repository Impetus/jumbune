package org.jumbune.deploy.cdh;
...
public class CDHDeployer extends YarnDeployer implements Deployer {
	...
	public String[] getRelativePaths(String versionNumber) {
		...
		for (String jar : ADDITIONAL_HADOOP_JARS) {
			if (jar.contains("hadoop")
				...
				strBuilder = new StringBuilder().append("-mapreduce").append(File.separator).append(jar);
				relativeJarPaths[i++] = strBuilder.toString();
			}
		}
