...
public final class DeployUtil {
	...
	private static final Logger DEBUG_LOGGER = LogManager
	...
	private static final String UPDATE_WAR_FILE = "/modules/jumbune-web-"
			+ Versioning.BUILD_VERSION + Versioning.DISTRIBUTION_NAME
			+ ".war WEB-INF/lib";
	private static final String UPDATE_AGENT_JAR = "/agent-distribution/jumbune-remoting-"
			+ Versioning.BUILD_VERSION
			+ Versioning.DISTRIBUTION_NAME
			+ "-agent.jar lib/";
	...
	private static final String UPDATE_WAR_CLASSES_FILE = "/modules/jumbune-web-"
			+ Versioning.BUILD_VERSION
			+ Versioning.DISTRIBUTION_NAME
			+ ".war WEB-INF/classes";
	 ...
	 * @throws IOException
	 */
	private static void getArgumentsFromPropertyFile(Map<String, String> argMap)
			throws IOException {
		if (argMap.containsKey(PROPERTY_FILE)) {
			...
			try {
				...
				CONSOLE_LOGGER.error("Unable to read file "
						+ argMap.get("propertyfilepath"));
	 ...
	 * 
	 * @param argMap
	 *            map containing arguments and their values
	 */
	...
	private static void printHelp() {
		CONSOLE_LOGGER
				.info("Usage: java -jar [Jumbune Jar File Name] [-verbose ] [Options]...\n"
	...
	public static void main(String[] args) throws InterruptedException,
		...
		if (argMap.get(PRIVATE_KEY_PATH) != null) {
			...
			if (!privateKeyFile.exists() || privateKeyFile.isDirectory()) {
				CONSOLE_LOGGER.error("Invalid private key file path ["
						+ privateKeyFile + "]");
				exitVM(1);
			}
		}
		...
		try {
			...
			if (hadoopDistributionType != null) {
				...
				if (distributionType.equalsIgnoreCase("Non-Yarn")) {
					...
					while (hadoopDistributionType.isEmpty()
							|| (!hadoopDistributionType.equalsIgnoreCase("a") && !hadoopDistributionType
									.equalsIgnoreCase("m"))) {
						CONSOLE_LOGGER
					...
					while (hadoopDistributionType.isEmpty()
							|| (!hadoopDistributionType.equalsIgnoreCase("a")
									&& !hadoopDistributionType
											.equalsIgnoreCase("h") && !hadoopDistributionType
										.equalsIgnoreCase("c"))) {
						CONSOLE_LOGGER
			...
			extractJarDirectories(new File(FoundPaths.get("<JUMBUNE.HOME>")),
					jarConnection);
			...
			updateJumbuneAndHadoopDistribution(FoundPaths.get("<JAVA.HOME>"),
					session, FoundPaths.get("<JUMBUNE.HOME>"),
					hadoopDistributionType, distributionType);

			CONSOLE_LOGGER.info("!!! Jumbune successfully deployed at ["
					+ FoundPaths.get("<JUMBUNE.HOME>") + "] !!!");
	...
	private static void serializeDistributionType(String distributionType,
			String HadoopDistribution) throws IOException {
		FileWriter writer = new FileWriter(new File(
				"./WEB-INF/classes/distributionInfo.properties"));
	...
	private static void createConfigurationFile(String hadoopType,
			String hadoopDistribution) throws IOException {
		String configurationpath;
		FileWriter writer = null;
		try {
			configurationpath = FoundPaths.get("<JUMBUNE.HOME>")
					+ File.separator + Constants.CONFIGURATION;
			File configuration = new File(configurationpath);
			if (!configuration.exists()) {
				configuration.mkdirs();
			}
			File clusterfile = new File(configurationpath + File.separator
					+ Constants.CLUSTER_INFO);
	...
	private static void checkJumbuneDirectoryCreation() {
		for (int j = 0; j < FOLDERS.length; j++) {
			final File f = new File(FoundPaths.get("<JUMBUNE.HOME>"),
					FOLDERS[j]);
			if (!DeployUtil.ensureDirectoryExists(f)) {
				CONSOLE_LOGGER.error("Error occurred while creating: "
						+ f.getAbsolutePath());
			}
		}
	}
	...
	private static Session getSession(Map<String, String> argMap)
			throws IOException {
		Console console = System.console();
		return validateUserAuthentication(console, argMap);
	}

	private static URLConnection performSanity() throws IOException,
			URISyntaxException {
		...
		if (!ensureDirectoryExists(jumbuneHome)) {
			CONSOLE_LOGGER
					.warn("Failed to create directory (may be already exist)"
							+ jumbuneHomeStr);
		}
		...
		CodeSource codeSource = DeployUtil.class.getProtectionDomain()
				.getCodeSource();
	 ...
	 */
	private static String getAndCheckDirectoryExistence(
			String enviromentVariable) {
		String directoryPath = null;
		File file = null;
		if (!System.getenv().containsKey((enviromentVariable))) {

			CONSOLE_LOGGER.info(enviromentVariable
					+ " not set as environment variable.");
			if (enviromentVariable.equals(JUMBUNE_ENV_VAR)) {
				CONSOLE_LOGGER
						.info("Please provide the absolute path to a folder where you want to deploy Jumbune");
			}
			directoryPath = SCANNER.nextLine().trim();
			file = new File(directoryPath);
			while (isNullOrEmpty(directoryPath)
					|| !DeployUtil.ensureDirectoryExists(file)) {
				CONSOLE_LOGGER
						.info("INVALID: Please provide a valid existing directory!!!");
				directoryPath = SCANNER.nextLine().trim();
				file = new File(directoryPath);
			}
		} else {
			...
			file = new File(directoryPath);
			CONSOLE_LOGGER.info(enviromentVariable + " linked to :- "
					+ directoryPath);
		}
	 ...
	 * @throws InterruptedException
	 */
	private static void updateJumbuneAndHadoopDistribution(String javaHomeStr,
			Session session, String jumbuneHomeStr,
			String hadoopDistributionType, String distributionType)
		...
		try {
			SessionEstablisher.fetchHadoopJarsFromNamenode(session, username,
					namenodeIP, hadoopHome, currentDir + WEB_FOLDER_STRUCTURE,
					hadoopDistributionType, distributionType);
		...
		String updateJumbuneWar = append(javaHomeStr, "/bin/", UPDATE_JAR,
				jumbuneHomeStr, UPDATE_WAR_FILE, "/");
		String updateJumbuneWarClasses = append(javaHomeStr, "/bin/",
				UPDATE_JAR, jumbuneHomeStr, UPDATE_WAR_CLASSES_FILE, "/");
		String updateAgentJar = append(javaHomeStr, "/bin/", UPDATE_JAR,
				jumbuneHomeStr, UPDATE_AGENT_JAR);
		String copyHadoopJarsToLib = append("cp -r ", currentDir,
				WEB_FOLDER_STRUCTURE, " ", FoundPaths.get("<JUMBUNE.HOME>"),
				Path.SEPARATOR, " ");
	...
	private static String getHadoopLocation(Session session,
			String hadoopDistributionType) throws JSchException, IOException {
		DEBUG_LOGGER.debug("Trying to locate Hadoop with echo $HADOOP_HOME");
		String hadoopHome = SessionEstablisher.getHadoopHome(session,
				SessionEstablisher.ECHO_HADOOP_HOME, "echo $HADOOP_HOME");
		DEBUG_LOGGER.debug("Hadoop location with echo $HADOOP_HOME ["
				+ hadoopHome + "]");
		if (hadoopHome == null || hadoopHome.trim().isEmpty()
			...
			if ((hadoopHome == null || hadoopHome.trim().isEmpty())
				...
				String llResponse = SessionEstablisher.getHadoopHome(session,
	...
	private static String getHadoopHome(String llResponse) {
		if (llResponse != null) {
			llResponse = llResponse.substring((llResponse.indexOf(">") + 1),
					llResponse.length());
	...
	private static void validateHadoopLocation(String hadoopHome) {
		if (hadoopHome == null || hadoopHome.trim().isEmpty()
				|| !hadoopHome.contains(File.separator)) {
			CONSOLE_LOGGER
					.info("Unable to find location of Hadoop! Please make"
	 ...
	 *         authentication.
	 */
	private static Session validateUserAuthentication(Console console,
			Map<String, String> argMap) throws IOException {
		char[] password = null;
		...
		do {
			...
			if (username == null) {
				CONSOLE_LOGGER.info("Username of the namenode machine -- > : ["
						+ user + "]");
			...
			if (argMap.get(PASSWORD) == null && privateKeyPath == null) {
				CONSOLE_LOGGER.info("Do we have passwordless SSH between ["
						+ masterNode + "] - [" + namenodeIP
						+ "] machines? (y)/(n)");
				...
				password = StringUtil.getPlain(argMap.get(PASSWORD))
						.toCharArray();
				sysexit = true;
			}

			// when the with password user authentication
			if (privateKeyPath == null && password == null) {
				String defaultPrivateKeyPath = "/home/" + user + "/.ssh/id_rsa";
				CONSOLE_LOGGER.info("Please provide private key file path ["
				...
				while (!privateKeyFile.exists() || privateKeyFile.isDirectory()) {
					CONSOLE_LOGGER
							.info("private key file should exist, please provide file path");
					privateKeyPath = SCANNER.nextLine().trim();
					privateKeyFile = new File(privateKeyPath);
				}
				DEBUG_LOGGER.debug("Authenticating username[" + username
						+ "], namenodeIP[" + namenodeIP
						+ "], and privatekeyPath[" + privateKeyPath + "]");
			...
			if (sysexit == true
					&& (tempSession == null || !tempSession.isConnected())) {
				CONSOLE_LOGGER
						.error("Failed to authenticate, check username and password");
				exitVM(1);
			}
	 ...
	 * @throws IOException
	 */
	public static boolean extractJarDirectories(final File destDir,
			final URLConnection jarConnection) throws IOException {
		DEBUG_LOGGER.debug("Extracting jar directories to [" + destDir
				+ "] location");
	...
	private static boolean copyJarResources(final File destDir,
			final JarFile jarFile, final JarEntry entry) throws IOException {
		...
		if (!entry.isDirectory()) {
			...
			try {
				...
				if (filename.indexOf("htfconf") != -1
						|| filename.indexOf(".properties") != -1
						|| filename.indexOf(".yaml") != -1) {
					DEBUG_LOGGER.debug("Replacing placeholders for ["
							+ filename + "]");
			...
			if (!DeployUtil.ensureDirectoryExists(f)) {
				throw new IOException("Error occurred while extracting : "
						+ f.getAbsolutePath());
			}
		}
		return true;
	 ...
	 * @throws IOException
	 */
	private static void includeAdditionalFileInLib(JarEntry entry,
		...
		for (String jarName : agentJars) {
			...
			fileToBeCopiedOnAgent = filename.substring(index + 1,
					filename.length());
			if (filename.contains(jarName)) {
				DEBUG_LOGGER.debug("Copied [" + fileToBeCopiedOnAgent
						+ "]  to Agent distribution");
				File fJar = new File(agentDestDir, "lib/"
						+ fileToBeCopiedOnAgent);
				InputStream entryInputStream = null;
				try {
					entryInputStream = jarFile.getInputStream(entry);
	 ...
	 * @return boolean
	 */
	private static boolean replacePlaceHolders(final File f,
		...
		try {
			...
			CONSOLE_LOGGER.error(
					"Failed to replace Placeholders for file " + f, ex);
			return false;
		}
		return true;
	}
	...
	private static String getJarContainingFolder(CodeSource codeSource,
			Class aClass) throws URISyntaxException,
