...
public class ClusterMonitoringCategories {
	...
	public String getNodeSpecificCategories() throws Exception {
		...
		if (isYarnEnable) {
			...
			if (!cluster.getTaskManagers().getHosts().contains(nodeIP)) {
				categoryInfo.getClusterWide().setJobTracker(null);
			}
		}
		...
		if (categoryInfo.getClusterWide().getNameNode() == null
				&& ((isYarnEnable && yarnCategoryInfo.getClusterWide().getResourceManager() == null)
						|| (!isYarnEnable && categoryInfo.getClusterWide().getJobTracker() == null))) {
	...
	private CategoryInfo getProfilerCategoryJson(Cluster cluster) throws Exception {
		...
		setClusterLevelDaemons(cluster, clusterWideInfo, dump, isYarnEnable);
		setNodeLevelDaemons(cluster, dump, levelJMXInfo, isYarnEnable);
	...
	private void setNodeLevelDaemons(Cluster cluster, ProfilerJMXDump dump, WorkerJMXInfo levelJMXInfo,
		...
		for (String workerHost : workers.getHosts()) {
			try {
				setYarnAndNonYarnDaemons(workerHost, workers, dump, levelJMXInfo, isYarnEnable, jmxPluginEnabled);
			} catch (Exception e) {
	...
	private void setYarnAndNonYarnDaemons(String host, Workers workers, ProfilerJMXDump dump,
			WorkerJMXInfo levelJMXInfo, boolean isYarnEnable, boolean jmxPluginEnabled) throws Exception {
		...
		if (isYarnEnable) {
			...
			levelJMXInfo.addToTaskTracker(dump.getAllJMXAttribute(JMXDeamons.TASK_TRACKER, host,
					workers.getTaskExecutorJmxPort(), jmxPluginEnabled));
		}
	...
	private void setClusterLevelDaemons(Cluster cluster, ClusterWideInfo clusterWideInfo, ProfilerJMXDump dump,
		...
		if (isYarnEnable) {
			...
			try {
				clusterWideInfo.setJobTracker(dump.getAllJMXAttribute(JMXDeamons.JOB_TRACKER,
						cluster.getResourceManager(), taskManagerJmxPort, cluster.isJmxPluginEnabled()));
			} catch (Exception e) {
				LOGGER.error("Unable to get Job Tracker stats for Node [" + cluster.getResourceManager() + "]", e.getMessage());
			}
		}
	}
