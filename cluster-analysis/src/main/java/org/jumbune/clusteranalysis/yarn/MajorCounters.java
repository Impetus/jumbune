...
public class MajorCounters {
	...
	/** The Constant LOGGER. */
	private static final Logger LOGGER = LogManager.getLogger(MajorCounters.class);
	...
	public Map<String, String> getMajorCounters(Cluster cluster) {
		...
		try {
			...
			LOGGER.error(UNABLE_TO_GET_NAMENODE_STATS, e);
		}
		...
		try {
			...
			LOGGER.error("Unable to get resource manager stats for cluster [" + cluster.getClusterName() + "]", e);
		}
	...
	private String getLastCheckpointTime(Map<String, String> nameNodeStats) {
	...
	public Map<String, String> getMaprCldbMetrics(Cluster cluster) throws Exception {
		...
		try {
			cldbStats = jmxDump.getAllJMXStats(JMXDeamons.CLDB, cluster.getNameNode(), "7220", false);
			...
			try {
				...
				long containersWithAtleastOneReplica = Long
						.parseLong(cldbStats.get(CLDB_SERVER_NO_OF_CONTAINERS_ONE_VALID_REPLICA));
				long containersWithoutAtleastOneReplica = numberOfContainers - containersWithAtleastOneReplica;

				map.put(NO_OF_CONTAINERS_WITHOUT_ATLEAST_ONE_REPLICA,
						String.valueOf(containersWithoutAtleastOneReplica));
		...
		if (resourceManagerStats != null) {
			...
			String resourceManagerRPC = resourceManagerStats.get(RPC_ACTIVITY_FOR_PORT54310_NUM_OPEN_CONNECTIONS);
