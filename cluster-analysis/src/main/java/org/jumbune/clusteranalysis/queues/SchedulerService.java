...
import org.jumbune.clusteranalysis.queues.beans.ChildQueues;
import org.jumbune.clusteranalysis.queues.beans.FairSchedulerQueueInfo;
import org.jumbune.clusteranalysis.queues.beans.FairSchedulerQueueInfoIntermediate;
...
public class SchedulerService {
	...
	public List<FairSchedulerQueueInfo> getFairSchedulerLeafQueues(final Cluster cluster) throws Exception {
		return SchedulerUtil.getFairSchedulerLeafQueues(fetchSchedulerInfo(cluster));
	}
	
	public List<CapacitySchedulerQueueInfo> getCapacitySchedulerLeafQueues(Scheduler scheduler) {
		return SchedulerUtil.getCapcitySchedulerLeafQueues(scheduler);
	}
	...
	private Scheduler parse(String sXml) throws Exception {
	...
	private List<FairSchedulerQueueInfo> getChildQueues(ChildQueues ch) {
		...
		List<FairSchedulerQueueInfoIntermediate> list = ch.getQueues();
		List<FairSchedulerQueueInfo> newList = new ArrayList<>();
		for (FairSchedulerQueueInfoIntermediate obj : list) {
